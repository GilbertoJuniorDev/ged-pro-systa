...
import {
  ...
  useUpdatePermission,
  useDeletePermission,
} from './use-permission-management';
...
describe('usePermissionsManagement', () => {
  ...
  it('should fetch and return permissions list', async () => {
    const perms = [makePermission(), makePermission({ id: 'perm-uuid-2', nome: 'DOCUMENTO_EDITAR' })];
    mockedApiClient.get.mockResolvedValue(perms);

    const { result } = renderHook(() => usePermissionsManagement(), { wrapper: createWrapper() });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(result.current.data).toHaveLength(2);
    expect(mockedApiClient.get).toHaveBeenCalledWith('/permissions', { token: 'test-token' });
  });
...
describe('useCreatePermission', () => {
  ...
  it('should include moduloId in payload when provided', async () => {
    const created = makePermission({ moduloId: 'mod-uuid-1', modulo: { id: 'mod-uuid-1', nome: 'Documentos', slug: 'documentos' } });
    mockedApiClient.post.mockResolvedValue(created);
...
describe('useUpdatePermission', () => {
  ...
  it('should call apiClient.patch with id and payload', async () => {
    const updated = makePermission({ descricao: 'Nova descrição' });
  ...
  it('should include moduloId: null to unlink module when updating', async () => {
    const updated = makePermission({ moduloId: null, modulo: null });
    mockedApiClient.patch.mockResolvedValue(updated);
...
describe('useDeletePermission', () => {
  ...
  it('should call apiClient.delete and show success toast', async () => {
    ...
    expect(mockedApiClient.delete).toHaveBeenCalledWith('/permissions/perm-uuid-1', { token: 'test-token' });
