...
function DeleteConfirm({ dossie, onConfirm, onCancel, isPending }: DeleteConfirmProps) {
  return (
    <div
    ...
    >
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
        <p className="text-slate-100 font-semibold mb-1">Remover dossiê?</p>
        <p className="text-slate-400 text-sm mb-6">
          O dossiê <span className="text-slate-200 font-medium">&quot;{dossie.nome}&quot;</span> será removido.
          Documentos vinculados a este dossiê ficarão avulsos (sem dossiê).
        </p>
...
export function DossieList() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useDossies(departamentoFiltro || undefined);
  ...
  if (isError) {
    return <p className="text-rose-400 text-sm py-4">Erro ao carregar dossiês.</p>;
  }

  const total = dossies?.length ?? 0;
  const ativos = dossies?.filter((d) => d.isActive).length ?? 0;
  ...
  return (
    <>
              ...
              : total === 0
                  ...
                  <tr>
                    <td colSpan={5} className="px-4 py-10 text-center">
                      <div className="flex flex-col items-center gap-2 text-slate-500">
                        ...
                        <p className="text-xs text-slate-600">Crie o primeiro dossiê para agrupar documentos sobre um mesmo assunto.</p>
                      </div>
                    </td>
                  </tr>
                )
                : dossies?.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-400">{departamentoNome(d.departamentoId)}</td>
      ...
      {editTarget && (
        <EditDossieDialog dossie={editTarget} onClose={() => setEditTarget(null)} />
      )}
      ...
      {deleteTarget && (
        <DeleteConfirm
          ...
          onConfirm={() => {
            deleteDossie.mutate(deleteTarget.id, {
