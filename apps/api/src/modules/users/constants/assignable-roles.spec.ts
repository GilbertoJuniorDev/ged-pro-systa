...
describe('getAssignableRoles', () => {
  it('should allow ADMIN and VIEWER when acting user is SUPER_ADMIN', () => {
    const result = getAssignableRoles(ROLE.SUPER_ADMIN);

    expect(result).toEqual([ROLE.ADMIN, ROLE.VIEWER]);
  });

  it('should allow only VIEWER when acting user is ADMIN', () => {
  ...
  it('should never include SUPER_ADMIN regardless of acting role', () => {
    ...
    expect(getAssignableRoles(ROLE.VIEWER)).not.toContain(ROLE.SUPER_ADMIN);
  });
});
