...
3. **End-to-end access scenarios** (manual, via a running API against the throwaway DB, or `/verify` if that skill's project script covers this flow):
   - Upload by a user *without* `DOCUMENTS_MANAGE_CONFIDENTIALITY` and not ADMIN/SUPER_ADMIN, requesting only the default (`confidencialidade` omitted or `RESTRITO`, no grants) → succeeds normally at `RESTRITO`, no grants, no permission check performed. The same user explicitly requesting `CONFIDENCIAL` (or any grants) → `403 ForbiddenException`, no document persisted with an elevated level (see Global Constraints — denial always throws, never silently downgrades).
