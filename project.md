...
6. [Banco de Dados e Schema Prisma](#6-banco-de-dados-e-schema-prisma)
...
| ORM | Prisma 6 | Type-safe, migrations automáticas, compatível com Postgres 17 |
...
│   └── @ged/database/                  # Schema Prisma + cliente gerado
│
├── infra/
│   ├── docker/
...
│   │
│   ├── database/
│   │   └── prisma.service.ts               # Prisma Client como NestJS Service
│   │
│   ├── config/
│   │   ├── app.config.ts
│   │   ├── auth.config.ts
...
│   └── e2e/
│
├── prisma/                                 # Symlink ou referência ao @ged/database
├── .env.example
├── nest-cli.json
├── tsconfig.json
├── package.json
...
├──────────────────────────────────────────────────────────────┤
│  INFRASTRUCTURE →  Repositories (Prisma), Storage, Cache     │
└──────────────────────────────────────────────────────────────┘
```
...
## 6. Banco de Dados e Schema Prisma

O schema Prisma centralizado reside em `packages/@ged/database/prisma/schema.prisma`.

```prisma
// packages/@ged/database/prisma/schema.prisma
...
```json
{
  ...
  "scripts": {
    ...
    "db:studio": "pnpm --filter=@ged/database prisma studio",
...
**Por quê:** Abstrai o acesso ao banco de dados. Os Services não conhecem o Prisma diretamente, apenas a interface do Repository. Facilita troca de ORM e mock em testes unitários.
...
| Colunas do banco | `snake_case` (via `@map`) | `created_at`, `file_name` |
